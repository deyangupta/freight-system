...
type Props = {
  ...
  sheetData: any[];
  standardColumn: { name: string, label: string }[];
...
const UploadModal: React.FC<Props> = ({
  showModal, setShowModal, file, fileUploaded, columns,
  mapping, sheetData, standardColumn, fileInputRef,
  handleFileUpload, handleMappingChange, handleSubmit, setFile
}) => {
      ...
      <div className={`bg-white rounded-lg p-6 w-full ${fileUploaded ? '' : 'max-w-lg'} max-h-[90vh] overflow-y-auto`}>
        ...
        {!fileUploaded && (
          <div onClick={() => fileInputRef.current?.click()}
            ...
            <svg className="w-12 h-12 text-blue-500 mb-2" fill="none" stroke="currentColor" strokeWidth="2"
                 viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4"/>
            </svg>
            <p className="text-sm">Click or drag file to this area to upload</p>
            <p className="text-xs text-gray-500 mt-1">PDF, XLSX, or CSV preferred.</p>
        ...
        <div className="sticky bottom-0 left-0 right-0 bg-white flex justify-end mt-6 space-x-2 p-4 shadow-md">
          <button onClick={() => {
            setShowModal(false);
            setFile(null);
          }} className="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-sm rounded">Cancel</button>
          <button className="px-4 py-2 bg-black text-white hover:bg-gray-800 text-sm rounded" onClick={handleSubmit}>
            Process Shipments
          </button>
        </div>
      </div>
